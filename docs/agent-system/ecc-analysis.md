...
1. **The Unadapted Copy-Paste Defect (`.agents/rules/build-error-resolver.md`):**
   ...
   - _Impact:_ Burns prompt context on every interaction while giving the agent incorrect instructions.
2. **Unanchored External Skill References:**
   ...
   - _Impact:_ The agent attempts to read missing files or halts with broken context.
...
1. **Skills (Primary Workflow Surface):** Reusable, on-demand procedure bundles containing prompts, execution patterns, and domain references. Loaded _only when needed_.
...
4. **Rules (Always-Loaded Constraints):** Global instructions loaded on _every single interaction_. ECC explicitly cautions against loading too many rules because every rule directly steals context window capacity and increases instruction drift.
