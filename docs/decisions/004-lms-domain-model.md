...
- **Single Monolithic Lesson Table**: Combining metadata and body content into a single table.
  _Rejected_: Degrades query performance for syllabus views and player sidebar navigation where lesson body is unnecessary.
- **Embedded Document Store (JSONB for Modules/Lessons)**: Storing the entire curriculum outline as a JSONB array on the `Course` model.
  _Rejected_: Eliminates relational foreign keys, complicates incremental progress tracking, prevents atomic transactions on individual lessons, and hinders SQL indexing.
- **Client-Driven Reorder Swaps**: Swapping adjacent indices on client requests.
  _Rejected_: Susceptible to race conditions, out-of-order networks, and gaps when multiple items are reordered.
